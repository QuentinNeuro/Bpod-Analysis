...
if Batch==1
    ...
    for i=1:size(ListFolders,1)
        ...
        try
        Pupillometry=Pupillometry_FIJI_Data_Reader_Embedded(thisFolder);
        catch
        disp([ListFolders(i,:) ' NOT ANALYZED']);
        end
    end
else
    Pupillometry=Pupillometry_FIJI_Data_Reader_Embedded(FoldDir);
end  
warning('on','all')

function Pupillometry=Pupillometry_FIJI_Data_Reader_Embedded(testBatch,testSave)
